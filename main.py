...
def ReqSquad(client_id, key, iv):
    fields = {1:2, 2:{1:int(client_id),2:"BD",4:1}}
    packet = create_packet(fields).hex()
    encrypted_packet = aes_encrypt(packet, key, iv)
    hlen = len(encrypted_packet) // 2
    return bytes.fromhex("0515000000" + dec_to_hex(hlen) + encrypted_packet)
...
def SEnd_InV(Nu , Uid , K , V):
    fields = {1: 2 , 2: {1: int(Uid) , 2: "BD" , 4: int(Nu)}}
    return GeneRaTePk(str(CrEaTe_ProTo(fields).hex()) , '0515' , K , V)
...
def Join_Room(room_id , K , V):
    fields = {1: 3, 2: {1: int(room_id), 8: {1: "IDC1", 2: 3000, 3: "BD"}, 9: "\x01\t\n\x12\x19 ", 10: 1, 12: b"\xff\xff\xff\xff\xff\xff\xff\xff\xff\x01\xff\xff\xff\xff\xff\xff\xff\xff\xff\x01", 13: 3, 14: 3, 16: "BD"}}
...
def request_skwad(idplayer, key, iv):
    import random
    same_value = random.choice([4096, 16384])
    fields = {
    1: 33,
    2: {
    1: int(idplayer),
    2: f"{os.getenv('SERVER', 'BD')}",
    3: 1,
    4: 1,
    5: bytes([1, 7, 9, 10, 11, 18, 25, 26, 32]),
    6: "iG:[C][B][FF0000] @mr.kalpha",
    7: 330,
    8: 1000,
    10: f"{os.getenv('SERVER', 'BD')}",
